describe('Products', () => {
  ...
  it('adds service successfully', () => {
    ...
    cy.get('[data-cy="categoryLi"]').click();
...
it('deletion of the product fails', () => {
  cy.visit('/en/products/20');

  cy.intercept('POST', '/en/products/20', {
    statusCode: 500,
...
//addeing to usedServices
it('added service to usedServices', () => {
  let sessionId;
...
it('didnt add service to usedServices', () => {
  let sessionId;
  cy.visit('/en/services/13');
